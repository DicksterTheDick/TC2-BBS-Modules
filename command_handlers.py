...
# NEW IMPORT: Import the BlackJack module to call its functions
import plugins.games.blackjack as bj

# Read the configuration for menu options
config = configparser.ConfigParser()
# Ensure config.ini exists in the same directory or provide the full path.
config.read('config.ini')

# Load menu options from config.ini
...
# NEW: Loading options for the Games menu
games_menu_items = config['menu']['games_menu_items'].split(',')


def build_menu(items, menu_name):
    """
    Constructs the string representation of a menu to display to the user.

    Args:
        items (list): List of menu item identifiers (e.g., 'Q', 'B', 'U', 'X').
        menu_name (str): The title of the menu (e.g., "💾TC² BBS💾").
    ...
    for item in items:
        stripped_item = item.strip()  # Remove whitespace
        if stripped_item == 'Q':
            menu_str += "[Q]uick Commands\n"
        elif stripped_item == 'B':
            if menu_name == "📰BBS Menu📰":
                menu_str += "[B]ulletins\n"
            # NEW: Specific display for Blackjack in the Games menu
            elif menu_name == "🎲Games Menu🎲":
                menu_str += "[B]lackjack 🃏\n"
            else:
                menu_str += "[B]BS\n"
        elif stripped_item == 'U':
            menu_str += "[U]tilities\n"
        # NEW: Added entry for the Games menu
        elif stripped_item == 'G':
            menu_str += "[G]ames 🎲\n"
        elif stripped_item == 'X':
            menu_str += "E[X]IT\n"
        elif stripped_item == 'M':
            menu_str += "[M]ail\n"
        elif stripped_item == 'C':
            menu_str += "[C]hannel Dir\n"
        elif stripped_item == 'J':
            menu_str += "[J]S8CALL\n"
        elif stripped_item == 'S':
            menu_str += "[S]tats\n"
        elif stripped_item == 'F':
            menu_str += "[F]ortune\n"
        elif stripped_item == 'W':
            menu_str += "[W]all of Shame\n"
    return menu_str

def handle_help_command(sender_id, interface, menu_name=None):
    """
    Handles the help command and display of various menus.
    ...
    Args:
        ...
        menu_name (str, optional): The name of the menu to display ('bbs', 'utilities', 'games').
                                  If None, displays the main menu. Defaults to None.
    """
    if menu_name:
        # Update user state to indicate the current menu
        ...
        # NEW: Handles the display of the Games menu
        ...
        # Display main menu with unread message count
        response = build_menu(main_menu_items, f"▓▒░The Off-Grid BBS!░▒▓ (✉️:{len(mail)})")
    send_message(response, sender_id, interface)

def get_node_name(node_id, interface):
    """
    Retrieves the long name of a node from its ID.

    Args:
        node_id (int): The numeric ID of the node.
        interface (meshtastic.stream_interface.StreamInterface): The Meshtastic interface object.

    Returns:
        str: The long name of the node, or "Node {node_id}" if not found.
    """
...
# NEW: Function to handle the start of the BlackJack game
def handle_blackjack_command(sender_id, interface):
    """
    Handles the user selecting Blackjack from the Games menu.
    Sets the user's state to BLACKJACK_GAME and prompts them to type 'deal'.

    Args:
        sender_id (int): The numeric ID of the sending node.
        interface (meshtastic.stream_interface.StreamInterface): The Meshtastic interface object.
    """
    # Set user state to indicate they are now in the BlackJack game context
    update_user_state(sender_id, {'command': 'BLACKJACK_GAME', 'step': 1})
    # Send a message prompting the user to type 'deal' to start the game
    send_message("Welcome to Blackjack! Type 'deal' to start a new game.", sender_id, interface)
...
def handle_exit_command(sender_id, interface):
    """
    Handles the exit command, resets the user's state.
    """
...
def handle_fortune_command(sender_id, interface):
    """
    Displays a random fortune from a file.
    """
    try:
        with open('fortunes.txt', 'r', encoding='utf-8') as file: # Added encoding
        ...
        send_message("Error: 'fortunes.txt' file not found.", sender_id, interface)
...
def handle_stats_steps(sender_id, message, step, interface):
    """
    Manages the steps for the Stats menu.
    """
...
def handle_bb_steps(sender_id, message, step, state, interface, bbs_nodes):
    """
    Manages the steps for the Bulletin Board menu.
    """
    ...
    if step == 1:
        ...
        if message.lower() == 'r':
            ...
            if board_name.lower() == 'urgent':
                ...
                logging.info(f"Checking permissions for node_id: {node_id} with allowed_nodes: {allowed_nodes}")  # Debugging
...
def handle_mail_steps(sender_id, message, step, state, interface, bbs_nodes):
    """
    Manages the steps for the Mail menu.
    """
...
def handle_wall_of_shame_command(sender_id, interface):
    """
    Displays a list of devices with low battery levels.
    """
...
def handle_channel_directory_command(sender_id, interface):
    """
    Displays the Channel Directory menu.
    """
...
def handle_channel_directory_steps(sender_id, message, step, state, interface):
    """
    Manages the steps for the Channel Directory menu.
    """
    message = message.strip()
    if len(message) == 2 and message[1] == 'x':
        message = message[0]
...
def handle_send_mail_command(sender_id, message, interface, bbs_nodes):
    """
    Handles the quick send mail command (SM,,).
    """
    try:
...
def handle_check_mail_command(sender_id, interface):
    """
    Handles the quick check mail command (CM).
    """
    try:
        sender_node_id = get_node_id_from_num(sender_id, interface)
        mail = get_mail(sender_node_id)
...
def handle_delete_mail_confirmation(sender_id, message, state, interface, bbs_nodes):
    """
    Handles mail deletion confirmation.
    """
    try:
...
def handle_post_bulletin_command(sender_id, message, interface, bbs_nodes):
    """
    Handles the quick post bulletin command (PB,,).
    """
    try:
        parts = message.split(",,", 3)
        if len(parts) != 4:
...
def handle_check_bulletin_command(sender_id, message, interface):
    """
    Handles the quick check bulletin command (CB,,).
    """
    try:
        ...
        # Safely find board_name in boards values and get its key
        found_board_key = next((key for key, value in boards.items() if value == board_name), None)
        if found_board_key is None:
            send_message(f"Invalid board name: '{board_name}'. Available boards are General, Info, News, Urgent.", sender_id, interface)
            return
        board_name = boards[found_board_key] # Use the canonical board name
...
def handle_read_bulletin_command(sender_id, message, state, interface):
    """
    Reads a specific bulletin message.
    """
    try:
...
def handle_post_channel_command(sender_id, message, interface):
    """
    Handles the quick post channel command (CHP,,).
    """
    try:
        parts = message.split("|", 3)
        if len(parts) != 3:
...
def handle_check_channel_command(sender_id, interface):
    """
    Handles the quick check channel command.
    """
    try:
        channels = get_channels()
        if not channels:
...
def handle_read_channel_command(sender_id, message, state, interface):
    """
    Reads a specific channel from the directory.
    """
    try:
...
def handle_list_channels_command(sender_id, interface):
    """
    Lists all available channels in the directory.
    """
    try:
        channels = get_channels()
        if not channels:
...
def handle_quick_help_command(sender_id, interface):
    """
    Displays the quick help commands.
    """
